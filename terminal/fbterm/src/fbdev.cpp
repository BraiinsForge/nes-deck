...
// Keep terminal cells clear of the Deck panel's rounded corners. The inset is
// applied in the physical portrait buffer, so it is 16 pixels on every edge
// after the 270-degree rotation as well.
static const u32 viewport_inset = 16;
...
FbDev *FbDev::initFbDev() {
  ...
  if (ioctl(fbdev_fd, FBIOGET_FSCREENINFO, &finfo) != 0 ||
      ioctl(fbdev_fd, FBIOGET_VSCREENINFO, &vinfo) != 0) {
    fprintf(stderr, "can't query frame buffer geometry!\n");
    close(fbdev_fd);
    fbdev_fd = -1;
    return 0;
  }
  ...
  /*
   * The Deck panel is physically portrait.  Its controller exposes a
   * 600x1280 RGB565 framebuffer, but only physical columns 0..479 are visible
   * in the enclosure.  Rotate that exact 480x1280 area to 1280x480.  Refuse
   * unknown modes instead of drawing off-screen or into the unused strip.
   */
  if (vinfo.xres != 600 || vinfo.yres != 1280 ||
      vinfo.bits_per_pixel != 16 || vinfo.xoffset != 0 ||
      vinfo.yoffset != 0 || finfo.line_length < 1200 ||
      (finfo.line_length & 1) != 0 ||
      finfo.smem_len < finfo.line_length * 1280 ||
      vinfo.red.offset != 11 || vinfo.red.length != 5 ||
      vinfo.red.msb_right != 0 || vinfo.green.offset != 5 ||
      vinfo.green.length != 6 || vinfo.green.msb_right != 0 ||
      vinfo.blue.offset != 0 || vinfo.blue.length != 5 ||
      vinfo.blue.msb_right != 0 || vinfo.transp.length != 0) {
    fprintf(stderr,
            "unsupported Deck framebuffer; expected 600x1280 RGB565 "
            "with a valid stride\n");
    close(fbdev_fd);
    fbdev_fd = -1;
    return 0;
  ...
  case 32:
    if (finfo.visual != FB_VISUAL_TRUECOLOR &&
      ...
      fprintf(stderr, "only support true-color/direct-color visual with "
                      "15/16/32bpp depth!\n");
      return 0;
    }
    break;
  ...
  default:
    fprintf(stderr,
            "only support frame buffer device with 8/15/16/32 color depth!\n");
    return 0;
  }

  FbDev *device = new FbDev();
  if (!device->mVMemBase) {
    delete device;
    return 0;
  }
...
FbDev::FbDev() {
  // Pre-rotation dimensions; Screen::createInstance swaps these to a
  // 1248x448 terminal viewport inside the full 1280x480 display.
  mWidth = 480 - 2 * viewport_inset;
  mHeight = 1280 - 2 * viewport_inset;
  mBitsPerPixel = vinfo.bits_per_pixel;
  mBytesPerLine = finfo.line_length;
  mFbMapBase = (u8 *)mmap(0, finfo.smem_len, PROT_READ | PROT_WRITE,
                          MAP_SHARED, fbdev_fd, 0);

  if (mFbMapBase == MAP_FAILED) {
    mFbMapBase = 0;
    mVMemBase = 0;
    fprintf(stderr, "can't map frame buffer memory!\n");
  } else {
    // A deterministic black surround prevents pixels from the previous UI
    // remaining visible in the rounded-corner padding.
    memset(mFbMapBase, 0, finfo.line_length * 1280);
    mVMemBase = mFbMapBase + viewport_inset * finfo.line_length +
                viewport_inset * 2;
  }

  if (mRotateType == Rotate0 || mRotateType == Rotate180) {
    bool ypan = (vinfo.yres_virtual > vinfo.yres && finfo.ypanstep &&
                 !(FH(1) % finfo.ypanstep));
...
FbDev::~FbDev() {
  if (mFbMapBase)
    munmap(mFbMapBase, finfo.smem_len);
  close(fbdev_fd);
...
const s8 *FbDev::drvId() { return finfo.id; }
...
void FbDev::setupPalette(bool restore) {
  if (finfo.visual == FB_VISUAL_TRUECOLOR || (!restore && !mPalette))
    return;
