...
Retro Deck is built on free software. The deployer installs upstream notices
and license texts at `/mnt/data/nes-deck/licenses`. Runtime packages carry
their own notices, while shared runtime libraries, the Go runtime used by the
checked-in `wireguard-go` userspace binary, Nixpkgs, and CC0 asset provenance
are collected by `nix/runtime-licenses.nix`. The separately pinned ECL runtime
carries ECL, ASDF/UIOP, Boehm GC, libatomic_ops, GMP, and glibc notices from the
exact source archives used in its build.
