//! Typed, bounded parsing and encoding for dashboard catalogs.
...
/// Maximum accepted size of one catalog document.
pub const MAXIMUM_CATALOG_BYTES: usize = 64 * 1_024;
/// Maximum combined built-in and uploaded entry count.
pub const MAXIMUM_GAMES: usize = 64;
/// Maximum byte length of a stable catalog identifier.
pub const MAXIMUM_IDENTIFIER_BYTES: usize = 48;
...
impl GameColor {
    ...
    /// Red, green, and blue components without a display-format assumption.
    #[must_use]
...
impl Catalog {
    ...
    pub fn parse(contents: &[u8]) -> Result<Self, CatalogError> {
        if contents.len() > MAXIMUM_CATALOG_BYTES {
    ...
    /// Insert an entry and keep supplemental catalogs ordered by system then title.
    ///
    /// # Errors
    ///
...
/// Catalog syntax or invariant failure.
...
impl CatalogError {
    const fn at_line(self, number: usize) -> Self {
        match self {
...
impl fmt::Display for CatalogError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InputTooLarge => formatter.write_str("catalog exceeds its size limit"),
            Self::NotUtf8 => formatter.write_str("catalog is not UTF-8"),
            Self::Line { number, reason } => {
                write!(formatter, "catalog line {number}: {reason}")
            }
            Self::InvalidField(reason) => formatter.write_str(reason),
            Self::TooManyEntries => {
        ...
        assert!(matches!(
            ...
            Some(entry)
                if entry.identifier() == "mario"
                    ...
                    && entry.color().components() == [215, 135, 135]
        ));
    }
...
one\tTWO\tnes\t/mnt/data/roms/nes/b.nes\t#FF5F00\n";
    ...
    #[test]
    fn game_colors_are_typed_canonical_and_lossless() {
        let color = GameColor::parse("#ff5f00");
        assert!(
            matches!(color, Ok(color) if color.to_string() == "#FF5F00" && color.components() == [255, 95, 0])
        );
